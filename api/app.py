...
# Adjust paths for model loading - look in current directory and parent directories
def find_model_path(filename):
    # Current directory (api folder)
    ...
    # Check in order: current dir, parent dir, relative paths
    paths_to_check = [
        api_path,
        parent_path,
        filename,
...
try:
    ...
    # For serverless, we'll handle this gracefully
    dtr = None
    preprocessor = None

# Flask app
app = Flask(__name__)
CORS(app, origins=["*"])  # Enable CORS for all origins (adjust in production)
...
def predict():
    try:
        if not dtr or not preprocessor:
...
try:
    ...
    print(f"Warning: MongoDB connection failed during startup: {e}")
